const STYLE_ID = 'arborenum-favorites-v2-style';
    ...
    if (node.kind === 'split') {
      const domTitle = document
        .querySelector<HTMLElement>(
          `.react-flow__node-praxis[data-id="${node.uid}"] .node-title`,
        )
        ?.textContent?.trim();
...
function activeUidFromDom(): number | undefined {
  const active = document.querySelector(
    '.react-flow__node-praxis .praxis-node.active',
  );
...
function makeTreeThumbnail(tree: FavoriteTree): SVGSVGElement {
  ...
  for (const point of layout.points) {
    ...
    circle.setAttribute(
      'class',
      `starred-thumb-node starred-thumb-${point.node.kind}`,
    );
...
function renderCard(tree: FavoriteTree, current: string | undefined): HTMLElement {
  ...
  if (exportMode) {
    ...
    checkbox.addEventListener('change', () =>
      setExportSelection(tree.id, checkbox.checked),
    );
  ...
  if (complete) {
    ...
    summary.textContent = `${accuracy} · ${counts.leaves} ${
      counts.leaves === 1 ? 'leaf' : 'leaves'
    }`;
  } else {
...
function renderExportToolbar(gallery: HTMLElement) {
  ...
  checkbox.type = 'checkbox';
  checkbox.checked =
    favorites.length > 0 && selectedForExport.size === favorites.length;
  checkbox.indeterminate =
    selectedForExport.size > 0 && selectedForExport.size < favorites.length;
  checkbox.addEventListener('change', () => {
...
function renderGallery() {
  ...
  subtitle.textContent =
    'Saved partial and completed trees. Rename them, reopen them, or export a collection.';
  ...
  if (favorites.length === 0) {
    ...
    empty.textContent =
      'No favorites yet. Build or modify a tree and press ☆ Star to save that exact state.';
...
function updateHeaderState() {
  ...
  const favoritesButton = document.querySelector<HTMLButtonElement>(
...
document.addEventListener(
  ...
  (event) => {
    ...
    if (
      ...
      target.closest('.react-flow__node-praxis') ||
