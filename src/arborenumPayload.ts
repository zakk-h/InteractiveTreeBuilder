...
function validateFeatureRegistry(
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    if (!Number.isInteger(entry.internalFeature) || entry.internalFeature !== i) {
      throw new Error(
        `featureRegistry must be index-addressable: expected internalFeature ${i}, got ${String(
          entry.internalFeature,
        )}.`,
      );
    }
...
function registryDisplayName(entry: ArborEnumFeatureRegistryEntry): string {
  return `${entry.originalName} <= ${entry.threshold}`;
}
...
function deriveContinuousGroups(
  ...
  for (const entry of registry) {
    if (entry.kind !== 'continuous_threshold' || entry.continuousGroup === null) {
      continue;
    }
...
function adaptArborEnumPayload(payload: ArborEnumBuilderPayload): void {
  if (!payload || typeof payload !== 'object') {
    throw new Error('window.ARBORENUM_BUILDER_PAYLOAD must be an object.');
  }
  ...
  const featureNames = registry.map(registryDisplayName);
  ...
  // main.tsx still consumes these names internally; they are not part of the
  // external payload contract anymore. The only embedded input is ArborEnum.
  const internalWindow = window as Window & Record<string, unknown>;
  internalWindow.PRAXIS_BUILDER_PAYLOAD = {
    graph: payload.graph,
    meta: derivedMeta,
  };
  internalWindow.PRAXIS_ANDOR_GRAPH = payload.graph;
  internalWindow.PRAXIS_ANDOR_META = derivedMeta;
}

const embeddedPayload = window.ARBORENUM_BUILDER_PAYLOAD;

if (embeddedPayload) {
  adaptArborEnumPayload(embeddedPayload);
}
